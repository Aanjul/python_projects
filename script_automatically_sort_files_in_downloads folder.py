#For refrence https://www.diffchecker.com/diff
    ...
    def on_created(self, event):
        if any(event.src_path.endswith(x) for x in file_folder_mapping):
            ...
            if not os.path.exists(parent):
                os.makedirs(parent)
                os.rename(event.src_path, os.path.join(parent, os.path.basement(event.src_path)))event_handler = DownloadedFileHandler()
                observer = Observer()
